...
const Gift= ({storeDetails}:any) => {
  ...
  const handleSampleDataByDate = async () => {
    try {
      ...
      const response = await getStockiestGiftDataByDateService(storeId);
  ...
  const handleSampleData = async () => {
    try {
      dispatch(setLoaderAction(true));
      const response = await getStockiestGiftService(storeId);
      dispatch(setLoaderAction(false));
      if (response.status === 200) {
  ...
  const handleSubmit = async () => {
    try {
      dispatch(setLoaderAction(true));
      const response = await postStockiestGiftSampleDataService(sampleData);
      dispatch(setLoaderAction(false));
      if (response.status === 200) {
        setModalOpen(false);
        handleSampleData();
