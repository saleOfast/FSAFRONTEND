...
const Gift= ({storeDetails}:any) => {
  ...
  const handleSampleDataByDate = async () => {
    try {
      ...
      const response = await getChemistGiftDataByDateService(storeId);
  ...
  const handleSampleData = async () => {
    try {
      dispatch(setLoaderAction(true));
      const response = await getChemistGiftService(storeId);
      dispatch(setLoaderAction(false));
      if (response.status === 200) {
  ...
  const handleSubmit = async () => {
    try {
      dispatch(setLoaderAction(true));
      const response = await postChemistGiftSampleDataService(sampleData);
      dispatch(setLoaderAction(false));
      if (response.status === 200) {
        setModalOpen(false);
        handleSampleData();
