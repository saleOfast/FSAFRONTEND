...
const Gift: React.FC<{
  storeDetails: { storeId: string; empId: string };
}> = ({ storeDetails }) => {
  const dispatch = useDispatch<AppDispatch>();
  const storeId = storeDetails?.storeId;
  console.log(storeId,"nnnnnn")
  ...
  const handleSampleDataByDate = async () => {
    try {
      ...
      const response = await getGiftOtherWorkPlaceDataByDateService(storeId);
  ...
  const handleSampleData = async () => {
    try {
      dispatch(setLoaderAction(true));
      const response = await getGiftsampleDataService(storeId);
      dispatch(setLoaderAction(false));
      if (response.status === 200) {
  ...
  const handleSubmit = async () => {
    try {
      dispatch(setLoaderAction(true));
      const response = await postGiftSampleDataService(sampleData);
      dispatch(setLoaderAction(false));
      if (response.status === 200) {
        setModalOpen(false);
        handleSampleData();
